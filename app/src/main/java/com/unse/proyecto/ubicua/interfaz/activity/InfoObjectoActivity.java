...
public class InfoObjectoActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (getIntent().getParcelableExtra(Utils.OBJ_INFO) != null) {
            mObjeto3D = getIntent().getParcelableExtra(Utils.OBJ_INFO);
        }
        ...
        mMapFragment.getMapAsync(new OnMapReadyCallback() {
            ...
            public void onMapReady(GoogleMap googleMap) {
                ...
                Marker marker = mGoogleMap.addMarker(new MarkerOptions()
                        .position(new LatLng(mObjeto3D.getLat(), mObjeto3D.getLog()))
                        .title(mObjeto3D.getNombre())
                        .icon(BitmapDescriptorFactory.fromBitmap(markerObject)));
                mGoogleMap.moveCamera(CameraUpdateFactory.newLatLngZoom(marker.getPosition(), 19));
            }
        });
        txtTitulo.setText(mObjeto3D.getNombre());
        Glide.with(imgIcon.getContext()).load(mObjeto3D.getUrlImg())
                .apply(new RequestOptions().diskCacheStrategy(DiskCacheStrategy.AUTOMATIC)
                        .error(R.drawable.rueda)
                        .placeholder(R.drawable.ic_3d_download)).into(imgIcon);
        if (mObjeto3D.getObjetoCaptura() != null){
            txtFecha.setText("Martes 30 de Julio de 2024 - 11:23");
            //txtFecha.setText(Utils.getFechaFormat(mObjeto3D.getObjetoCaptura().getFecha()));
        }
    }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.cardAlgoritmo:
                DialogoAlgoritmo dialogoAlgoritmo = new DialogoAlgoritmo(getApplicationContext(), mObjeto3D);
            ...
            case R.id.cardModoRA:
                ...
                intent.putExtra(Utils.OBJ_INFO, mObjeto3D);
